...
contract StrategyUmamiGlpVault is Strategy {
    string public name;
    IVault4626 public immutable vault;
    IERC20 public immutable asset;

    constructor(address _strategyHelper, address _vault, address _asset)
        Strategy(_strategyHelper)
    {
        vault = IVault4626(_vault);
        asset = IERC20(_asset);
        name = string(abi.encodePacked("Umami GLP Vault ", asset.symbol()));
    }
